...
app.message(/@([a-zA-Z0-9_-]+)/, async ({ message, say, context }) => {
  ...
  const mentions = [...message.text.matchAll(mentionRegex)].map(match => match[1]);
...
app.command('/group-create', async ({ command, ack, respond }) => {
  ...
  await respond(`Group \`@${groupName}\` created/updated with ${userIds.length} members.`);
});
...
app.command('/group-list', async ({ command, ack, respond }) => {
  ...
  if (groupNames.length === 0) {
    ...
    const groupList = groupNames.map(name => {
